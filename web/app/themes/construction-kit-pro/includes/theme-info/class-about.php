...
if ( ! class_exists( 'Construction_Kit_About' ) ) {
	...
	class Construction_Kit_About {
		...
		/**
		 * Singleton instance of Construction_Kit_About.
		 *
		 * @var Construction_Kit_About $instance Construction_Kit_About instance.
		 */
		private static $instance;

		/**
		 * Main Construction_Kit_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Construction_Kit_About ) ) {
				self::$instance = new Construction_Kit_About;
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'construction-kit-about', get_template_directory_uri() . '/includes/theme-info/css/about.css', array(), '2.0.2' );
				wp_enqueue_script( 'construction-kit-about', get_template_directory_uri() . '/includes/theme-info/js/about.js', array( 'jquery' ), '2.0.2' );
				$js_vars = array(
					'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
				);
				wp_localize_script( 'construction-kit-about', 'ConstructionKitAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'construction-kit' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'construction-kit' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'construction-kit' );
									break;
							}
							?>
		...
		public function free_pro() {
		    ...
		    if ( ! empty( $free_pro ) ) {
		        ...
		        if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
		            ...
		            echo '<tr>';
		            echo '<th></th>';
		            echo '<th>' . esc_html__( 'Free','construction-kit' ) . '</th>';
		            echo '<th>' . esc_html__( 'Pro','construction-kit' ) . '</th>';
		            echo '</tr>';
		            echo '</thead>';
		            echo '<tbody>';
		            ...
		            echo '<td></td>';
		            echo '<td colspan="2"><a href="https://wpcharms.com/item/construction-kit-pro/" target="_blank" class="button button-primary button-hero">Construction Kit Pro</a></td>';
		            echo '</tr>';
